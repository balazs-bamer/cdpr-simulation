# secondary input in the first hidden layer (e.g. Lilicrap et. al. 2016)
...
class MLP2(bc.GraphStructure):

    def __init__(self, dtype, *param, fn):
        super(MLP2, self).__init__(dtype)
        assert len(fn.input_names) == 2 and len(fn.output_names) == 1, "The function is not compatible with MLP2"
        ...
        top = tf.concat([top, self.input2], axis=1)

        layer_n = 1

        for dim in dimension[3:-1]:
