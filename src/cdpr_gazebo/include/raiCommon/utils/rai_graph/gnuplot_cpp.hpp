...
class Figure {
 ...
 public:
  ...
  template<typename Dtype>
  void appendData(Dtype *x, Dtype *y, int dataLen, const std::string& name) {
...
//    assert_false(currentFigure_ == figureType3D, "you are plot a 2D curve on 3D plots");
  ...
  /* adding a curve and standard deviation to a graph
    ...
    */
  template<typename Dtype>
  void appendDataAndStd(Dtype *x, Dtype *y, Dtype *std, int dataLen, const std::string& name, const std::string& style) {
